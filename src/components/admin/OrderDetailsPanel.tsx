...
interface OrderDetailsPanelProps {
  order: OrderRecord;
  onClose?: () => void; // Optional: Desktop view might not need a close button in the panel itself
  onStatusChange: (orderId: string, nextStatus: OrderStatus, paymentCollectedMethod?: string) => Promise<void>;
  onOrderUpdate?: () => void;
  className?: string; // Allow passing layout styles
}
...
export function OrderDetailsPanel({ order, onClose, onStatusChange, onOrderUpdate, className = '' }: OrderDetailsPanelProps) {
  ...
  // Revalidate order on open/change using per-order ETag
  useEffect(() => {
  ...
  return (
    <div className={`flex h-full flex-col bg-white dark:bg-slate-900 ${className}`}>
        {/* Header */}
        ...
        {/* Footer Actions (Only show Close if passed) */}
        {onClose && (
