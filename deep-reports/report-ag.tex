...
\begin{description}[style=standard, leftmargin=1cm, font=\bfseries\color{emerald}]
    ...
    \item[PBKDF2] \textbf{Password-Based Key Derivation Function 2.} A cryptographic algorithm used to hash passwords. It applies a function (like SHA-256) thousands of times to make it slow and expensive for hackers to guess passwords.
\end{description}
...
\begin{itemize}
    ...
    \item \textbf{Styling:} TailwindCSS 3.4 (Emerald Palette) with Poppins/Inter fonts.
    \item \textbf{Behavior:} It handles all routing client-side (`react-router-dom`) with routes for Catalog, Checkout, Orders, and Admin consoles.
    \item \textbf{State:} Uses React Context for global state (Auth, Cart, Products) and `localStorage` for persistence (Cart, Theme, Locale).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Runtime:} All logic runs inside the Cloudflare Workers runtime, which provides the global `env` object containing bindings.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{\texttt{ORDERS\_DB} (The Grand Ledger):} The D1 Database.
        \begin{itemize} 
            \item Preview ID: \texttt{756fd361-4883-4202-bccd-db469354c8f8} (\texttt{order\_ieeja\_orders\_preview})
            \item Production ID: \texttt{0eaea965-4467-4aee-89a9-28aed83e4c79} (\texttt{order\_ieeja\_orders})
        \end{itemize}
    \item \textbf{\texttt{LOGIN\_RATE\_LIMIT\_KV} (The Bouncer):} Tracks login failures per IP.
        \begin{itemize}
            \item Preview ID: \texttt{f059e5f6774f4e7ca9db0d7bf80841fd}
            \item Production ID: \texttt{5e8638dfbd414219947a22aa476944cf}
        \end{itemize}
    \item \textbf{\texttt{AUTH\_SECRET} (The Royal Seal):} A symmetric key ($\ge$ 16 chars) used to sign tokens. This is injected as an environment variable.
\end{enumerate}

\section{Shared Contracts: The Common Tongue}
\begin{itemize}
    \item \textbf{File:} \texttt{shared/order-schema.js}
    \item \textbf{Purpose:} Defines the "shape" of a valid order.
    \item \textbf{Usage:} Imported by both the Frontend (`src/utils/checkout.ts`) and Backend (`functions/order.js`).
    \item \textbf{Benefit:} Ensures that if the frontend thinks an order is valid, the backend will likely agree. It prevents "validation drift" where the client allows something the server rejects.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Algorithm:} HS256 (HMAC with SHA-256).
    \item \textbf{Claims:} `sub` (User ID), `role`, `status`, `token_version`, `type: "access"`.
    \item \textbf{Purpose:} Grants access to protected API endpoints.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Lifespan:} 7 days.
    \item \textbf{Security Attributes:} \texttt{SameSite=Lax}, \texttt{Secure}, \texttt{Path=/auth}.
    \item \textbf{Purpose:} Used solely to get a new Access Token when the old one melts. It is never exposed to client-side JavaScript.
\end{itemize}
...
\textbf{Our Solution:}
1. Every user in the database has a \texttt{token\_version} integer (default 1).
2. This number is baked into their tokens.
3. To logout/ban a user, we increment the number in the database to 2 via the `/auth/revoke` endpoint.
...
\subsection{Rate Limiting (The Bouncer)}
\begin{itemize}
    \item \textbf{Mechanism:} KV Storage (`LOGIN_RATE_LIMIT_KV`).
    \item \textbf{Key:} `rl:login:<IP_ADDRESS>`.
    \item \textbf{Policy:} 5 failed attempts per minute.
    \item \textbf{Penalty:} If exceeded, the IP is blocked for 15 minutes.
    \item \textbf{Response:} \texttt{429 Too Many Requests} with a \texttt{Retry-After} header.
    \item \textbf{Fallback:} If the KV binding is missing, the system logs a warning but allows the login (fail-open for availability).
\end{itemize}

\subsection{Role-Based Access Control (RBAC)}
The `requireAuth` middleware enforces strict role checks:
\begin{itemize}
    \item \textbf{Public:} `/products`, `/auth/*`
    \item \textbf{Customer:} `/order` (POST/GET own), `/account/*`
    \item \textbf{Rider:} `/rider`, `/order` (PATCH status)
    \item \textbf{Admin:} `/admin/*`, `/config`, `/order` (PATCH status, GET all)
\end{itemize}

\begin{critiquebox}{Password Hashing}
We use **PBKDF2-SHA256** with 100,000 iterations.
...
\begin{itemize}
    ...
    \item \texttt{customer\_phone}: The phone number used for the order.
\end{itemize}
...
\subsection{1. Orders (\texttt{orders})}
The central entity.
\begin{itemize}
    \item \textbf{Columns:} \texttt{id} (UUID), \texttt{status} (pending/confirmed/etc), \texttt{total\_amount} (REAL), \texttt{items\_json}, \texttt{user\_id}, \texttt{delivery\_address\_id}.
    \item \textbf{Indexes:} \texttt{created\_at} (for feeds), \texttt{status} (for filtering), \texttt{user\_id} (for customer history).
\end{itemize}

\subsection{2. Products (\texttt{products})}
The catalog.
\begin{itemize}
    \item \textbf{Columns:} \texttt{id}, \texttt{name}, \texttt{price} (REAL), \texttt{stock\_quantity} (INTEGER), \texttt{is\_active} (BOOLEAN).
    \item \textbf{Constraint:} \texttt{CHECK (stock\_quantity >= 0)} ensures we never have negative stock.
\end{itemize}

\subsection{3. Users (\texttt{users})}
The identity store.
\begin{itemize}
    \item \textbf{Columns:} \texttt{phone} (Unique), \texttt{password\_hash}, \texttt{role}, \texttt{token\_version}.
    \item \textbf{Profile:} \texttt{display\_name}, \texttt{primary\_address\_json} (Cached default address).
\end{itemize}

\section{The Migrations Timeline}
\begin{itemize}
    \item **0001-0003:** Foundation (Orders, Config).
    \item **0004-0005:** Identity (Users, Addresses).
    \item **0007:** Inventory Seed (280 SKUs).
    \item **0011:** Monetary Precision Fix (INTEGER -> REAL) and Stock Constraint.
    \item **0012:** Performance Tuning (Added \texttt{updated\_at} for ETags).
\end{itemize}
...
1. **Upsert Address:** Save new address to user profile (if authenticated).
2. **Insert Order:** Create the record with status `pending`.
3. **Insert Items:** Log individual line items into `order_items`.
...
The order moves through a lifecycle managed by Admins and Riders:
\begin{enumerate}
    \item \textbf{Pending:} Created by the customer.
    \item \textbf{Confirmed:} Accepted by the shop.
    \item \textbf{OutForDelivery:} Picked up by the rider.
    \item \textbf{Delivered:} Handed to the customer.
    \item \textbf{Cancelled:} Terminated (Note: Stock is NOT automatically restored).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Headers:} \texttt{Cache-Control: public, max-age=60, stale-while-revalidate=600}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Headers:} \texttt{Cache-Control: private, max-age=0, must-revalidate}.
\end{itemize}
...
\begin{itemize}
    \item **Indexes:** We heavily index `created_at` and `status` on the `orders` table to ensure the Admin Dashboard loads instantly even with thousands of orders.
    \item **Lightweight Queries:** The ETag queries only ask for `COUNT` and `MAX(updated_at)`, which are extremely fast for SQLite to compute, avoiding the need to load the full dataset to check for freshness.
\end{itemize}
...
\begin{itemize}
    ...
    \item It generates a JSON file \texttt{functions/\_inventory-data.json}.
    \item \textbf{Why?} It ensures the code always has access to the "Truth" of the inventory definition, seeded into the database during migration 0007.
\end{itemize}
...
\begin{itemize}
    ...
    \item **Process:** 
        1. Runs `build:inventory`.
        2. Builds React app to \texttt{dist/}.
        3. Pushes assets and functions to Cloudflare Pages.
        4. Binds to the Production D1 Database.
\end{itemize}

\section{Database Management Scripts}
We built a suite of custom scripts in `package.json` to manage D1:
\begin{itemize}
    \item \texttt{db:pull}: Downloads the remote database to your local machine for debugging.
    \item \texttt{db:migrate}: Applies new schema changes (migrations) to the database.
    \item \texttt{logs:tail}: Streams real-time logs from the production server to your terminal.
\end{itemize}
...
We have no centralized logging (like Datadog or Sentry). If a user faces a bug, we only know if they call us. We cannot see error rates spiking in real-time. We rely on `wrangler pages deployment tail` which is ephemeral.
\end{critiquebox}
...
\begin{itemize}
    ...
    \item **Pagination:** The Orders page loads up to 1000 orders. For a heavy user, this will eventually become slow.
\end{itemize}
